...
object TimeUsage extends TimeUsageInterface {
  ...
  def row(line: List[String]): Row = {
      val str = line.head
      val dbl = line.tail.map(x => x.toDouble)
      Row.fromSeq(str::dbl)
    }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def colControl(colName: String, group: List[String]): Boolean = {
      if(group.isEmpty) false
      else {
        if(colName.startsWith(group.head)) {
          //println(colName + " == " + group.head)
          true
        } else colControl(colName, group.tail)
        //case x::Nil => if(colName startsWith x) true else false
      }
    }
    val colAcc = (List.empty[Column], List.empty[Column], List.empty[Column])

    def colRec(colNames: List[String], acc: (List[Column], List[Column], List[Column])): (List[Column], List[Column], List[Column]) = {
      if (colNames.isEmpty) acc else {
        if (colControl(colNames.head, primary)) colRec(colNames.tail, (col(colNames.head)::acc._1, acc._2, acc._3))
        else if (colControl(colNames.head, work)) colRec(colNames.tail, (acc._1, col(colNames.head)::acc._2, acc._3))
        else if (colControl(colNames.head, other)){
          println(acc._1)
          colRec(colNames.tail, (acc._1, acc._2,col(colNames.head)::acc._3))
        }
        else colRec(colNames.tail, (acc._1,acc._2,acc._3))
      }
    }
    colRec(columnNames,colAcc)
  }
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String = {
    ...
    val work = "ROUND(AVG(work), 1) AS work"
    val other = "ROUND(AVG(other), 1) AS other"
    s"SELECT working, sex, age, $primaryNeeds, $work, $other FROM $viewName GROUP BY working, sex, age ORDER BY working, sex, age"
  }
  // s"SELECT primaryNeeds, work, other FROM $viewName"
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(x => TimeUsageRow(x.getAs("working"),
      x.getAs("sex"),
      x.getAs("age"),
      x.getAs("primaryNeeds"),
      x.getAs("work"),
      x.getAs("other")))
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(x => (x.working, x.sex, x.age))
      ...
      .map{ case ((wing,s,a),p,w,o) => TimeUsageRow(wing,s,a,p,w,o) }
      .orderBy("working", "sex", "age")//(string,string,string),double,double,double
  }
}
